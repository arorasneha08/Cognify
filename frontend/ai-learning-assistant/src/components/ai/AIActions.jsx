...
const AIActions = () => {
  ...
  const handleGenerateSummary = async () => {
    setLoadingAction("summary");
    try {
      const response = await aiService.generateSummary(document_id);
  ...
  const handleExplainConcept = async (e) => {
    ...
    try {
      const response = await aiService.explainConcept(document_id, concept);
      console.log("Explain API response:", response);
      ...
      <div className="bg-white/80 backdrop-blur-xl border border-slate-200/60 rounded-2xl shadow-xl shadow-slate-200/50 overflow-hidden">
        ...
        <div className="px-6 py-5 border-b border-slate-200/60 bg-linear-to-br from-slate-50/50 to-white/50">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-linear-to-br from-emerald-500 to-emerald-600 shadow-purple-500/25 flex items-center justify-center">
        ...
        <div className="p-6 space-y-6">
          ...
          >
            <div className="max-h-[60vh] overflow-y-auto prose prose-sm max-w-none prose-slate">
              <MarkdownRenderer
                content={modalContent || "No content available"}
              />
            </div>
          </Modal>
        </div>
