    ...
    <div className="max-w-5xl mx-auto">
      ...
      <div className="mb-6">
        <Link
          ...
          className="group inline-flex items-center text-sm font-medium text-slate-600 hover:text-emerald-600 transition-colors duration-200"
        >
          <ArrowLeft
      ...
      <div className="bg-white/80 backdrop-blur-xl border-2 border-slate-200 rounded-2xl shadow-xl shadow-slate-200/50 p-8 mb-8">
        <div className="text-center space-y-6">
          <div className="inline-flex items-center justify-centerw-15 h-15 rounded-2xl bg-linear-to-br from-emerald-100 to-teal-100 shadow-lg shadow-emerald-500/20 p-5">
          ...
          <div>
            ...
            <div
              className={`inline-block text-5xl font-bold bg-linear-to-r ${getScoreColor(score)} text-transparent bg-clip-text mb-2`}
            >
              {score}%
            </div>
      ...
      <div className="space-y-6">
        <div className="flex items-center gap-3 mb-5">
          <BookOpen className="w-5 h-5 font-semibold" strokeWidth={2} />
        ...
        {detailedResults.map((result, index) => {
          ...
          const correctAnswerIndex = result.correctAnswer.startsWith('0') ? parseInt(result.correctAnswer.substring(1)) - 1 : result.options.findIndex((option) => option === result.correctAnswer);

          const isCorrect = result.isCorrect;

          return (
            <div className="bg-white/80 backdrop-blur-xl border-2 border-slate-200 rounded-2xl shadow-lg shadow-slate-200/50 p-6" key={index}>
              ...
              <div className="space-y-3 mb-4">
                {result.options.map((option , optionIdx) => {
                  const isCorrectOption = optionIdx === correctAnswerIndex;
                  const isUserAnswer = optionIdx === userAnswerIndex;
                  const isWrongAnswer = isUserAnswer && !isCorrect ;

                  return (
                    <div className={`relative px-4 py-3 rounded-lg border-2 transition-all duration-200 ${isCorrectOption ? 'bg-emerald-50 border-emerald-300 shadow-lg shadow-emerald-500/10' : isWrongAnswer ? 'bg-rose-50 border-rose-300' : 'bg-slate-50 border-slate-200'}`} key={optionIdx}>
      ...
      <div className="mt-8 flex justify-center">
        ...
        <button className="group relative px-8 h-12 bg-linear-to-r from-emerald-500 to-teal-500 hover:from-emerald-600 hover:to-teal-600 text-white font-semibold text-sm rounded-xl shadow-lg shadow-emerald-500/25 transition-all duration-200 active:scale-95 overflow-hidden">
          ...
          <div className="absolute inset-0 bg-linear-to-r from-white/20 to-white/0 -translate-x-full group-hover:translate-x-0 transition-transform duration-700"/>
        </button>
        </Link>
      </div>
    </div>
